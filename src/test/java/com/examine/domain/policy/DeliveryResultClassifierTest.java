...
class DeliveryResultClassifierTest {
    ...
    @Test
    void success2xx() {
        DeliveryResult result = classifier.classify(HttpOutcome.response(200, Map.of(), 0L));
    ...
    void rateLimited429WithRetryAfter() {
        DeliveryResult result = classifier.classify(HttpOutcome.response(429, Map.of("Retry-After", "10"), 0L));
    ...
    void rateLimited429WithoutRetryAfter() {
        DeliveryResult result = classifier.classify(HttpOutcome.response(429, Map.of(), 0L));
        assertTrue(result instanceof DeliveryResult.RateLimited);
    ...
    void serverError5xxIsRetryable() {
        DeliveryResult result = classifier.classify(HttpOutcome.response(503, Map.of(), 0L));
    ...
    void clientError4xxIsNonRetryable() {
        DeliveryResult result = classifier.classify(HttpOutcome.response(400, Map.of(), 0L));
        assertTrue(result instanceof DeliveryResult.NonRetryableFailure);
    }

    @Test
    void socketTimeoutIsRetryable() {
        DeliveryResult result = classifier.classify(HttpOutcome.failure(new SocketTimeoutException("timeout"), 0L));
        assertTrue(result instanceof DeliveryResult.RetryableFailure);
    }

    @Test
    void connectExceptionIsRetryable() {
        DeliveryResult result = classifier.classify(HttpOutcome.failure(new ConnectException("refused"), 0L));
        assertTrue(result instanceof DeliveryResult.RetryableFailure);
    }
    ...
    void wrappedNetworkErrorIsRetryable() {
        ...
        DeliveryResult result = classifier.classify(HttpOutcome.failure(wrapper, 0L));
        assertTrue(result instanceof DeliveryResult.RetryableFailure);
    }

    @Test
    void nonRetryableException() {
        DeliveryResult result = classifier.classify(HttpOutcome.failure(new IllegalArgumentException("bad config"), 0L));
        assertTrue(result instanceof DeliveryResult.NonRetryableFailure);
    }
}
